...
def back():
	login()
def contact():
	os.system('xdg-open https://m.facebook.com/profile.php?id=100003713692609&eav=Afa_neY4ozyk9RbBHQdWFwVqDMTMcdXdGbs2D63g_Oe4Sf6_4Dpp7v93A81KCFkxNOQ&paipv=0&_rdr')
	back()
def linex():
	print('\033[1;37m----------------------------------------------')
...
\033[1;37m=========================
 AUTHOR     : BIBASH
 GITHUB     : Bibashthakuri7
 FACEBOOK   : Bibash thakuri
...
def approval():
  ...
  print ("""\033[1;37m----------------------------------------------""")
  try:
    httpCaht = requests.get("https://github.com/Bibashthakuri7/publick/blob/main/Approval.txt").text
    if id in httpCaht:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {P}[BIBASH-DW]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
